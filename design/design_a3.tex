...
\section{Front-End}
This section describes the front-end of the ADIN INSPECTOR - the UI elements the GUI consists of, and how states are handled. A series of final UI design mockups are presented under UI Design Mockups section, whereas an overview of the GUI classes can be seen in \autoref{fig:gui_overview}.\\
...
\begin{figure}[hp]
	\begin{adjustbox}
		{addcode={\begin{minipage}{\width}}
					...
					\end{minipage}}
			,center}
		\fbox{\includegraphics[max size={\textwidth}{0.4\textheight}]{images/loginNew.PNG}}
	\end{adjustbox}
\end{figure}
...
\begin{figure}[h!]
	\centering
		\includegraphics[max size={1.2\textheight}{\textwidth}]{images/overview.png}
		\caption{%
...
\section{Back-End}
This section deals with the back-end of the ADIN INSPECTOR. How the system deals with client http calls, and how kafka interacts with the system.
An overview of the system can be seen in \autoref{fig:class_back_end}. Smaller sections have been expanded in \autoref{fig:class_back_end_init}, \autoref{fig:class_back_end_mongo}, \autoref{fig:class_back_end_hub}.
...
\begin{itemize}
	...
	\item[•]MongoConsumer
	      ...
	      \begin{itemize}
		      ...
		      \item[-]getTopicsForProcessing
		            \\parameters: none
		            \\returns: ArrayList<String>
		            \\This convenience method returns a list of all topics to process, excluding internal Kafka topics and real-time data since the processing call is done elsewhere, as strings.
	...
	\item[•]MongoClientMediator
	      ...
	      \begin{itemize}
		      ...
		      \item[-]addRecordToCollection
		            \\parameters: Record record - a record to add to the collection
		            \\String collection - name of the collection it should be added to.
		            \\returns: void
		            \\Converts the record to a bson document and uses the mongoAPI to insert it into the database.

		      \item[-]addRecordToCollection
		            \\parameters: Document record - a record to add to the collection
		            \\String collection - name of the collection it should be added to.
		            \\returns: void
		            \\Takes a bson Document containing a record and uses the mongoAPI to insert it into the database.

		      \item[-]addRecordsToCollection
		            \\parameters: Array of records - the records to be added to a collection
		            \\String name of the collection it should be added to.
		            \\returns: void
		            \\for each oneof the members of the array call addRecordToCollection
		      \item[-]addRecordsToCollection
		            \\parameters: ArrayList of Documents - the records to be added to a collection
		            \\String name of the collection it should be added to.
		            \\returns: void
		            \\for each oneof the members of the array call addRecordToCollection
		      ...
		      \item[-]getRecordsInRange
		            \\parameters: String, name of the collection to query
		            \\Object, key of the parameter used for filtering
		            \\Object start and end ranges for the filtering
		            \\returns: String array containing all entries of the collection within that range
		            \\this Method is very general to allow for flexibility.For example by letting the key be, SourceIPaddresses, or a timeStamp.
...
\newpage
	...
	\item[•]PacketRecordDesFromKafka
	      ...
	      \begin{itemize}
		      \item[-]constructor
		            \\parameters: initializers for all attributes
	...
	\item[•] AddressesAndLinks
	      \\Implements IAggregator. This aggregator produces a document containing a list of network nodes that show up in a collection and a list of connections between the nodes.
Of importance is that nodes are unique while links do not have to be.
\begin{verbatim}
...
\end{verbatim}
	      ...
	      \begin{itemize}
		      \item[-]processData
		            \\parameters: ArrayList records - the records to be processed
			    \\returns: ArrayList<Document>
	      \end{itemize}


	\item[•] LoginFailureException
	      \\Exceptions of this class are thrown by MongoClientMediator and MongoClientConsumer when login into the mongo database was not possible; usually due to invalid username or password.
	      Methods
	      \begin{itemize}
		      \item[-]constructor
		            \\parameters: String errorMessage - the error message to be reported
	      \end{itemize}
	...
	\item[•]Hub
	      ...
	      \begin{itemize}
		      ...
		      \item[-]login
		            \\parameters: Session session - the current session
		            \\String username - the user to log in
		            \\String password - the user's password
		            \\returns: String
		            \\If the username and password are valid, create a new database user session with the user logged in and return a authentication token.
			      If the login fails, return an empty string.
				
		      \item[-]authenticate
		            \\parameters: Session session - the current session
		            \\String token - the authentication token to use
		            \\returns: boolean
		            \\ Return true if this token corresponds to a valid, currently logged in user session, otherwise return false.
		      ...
		      \item[-]getCollection
		            \\parameters: Session session - the current session
		            \\String collection - name of a collection
		            \\returns: String array containing all entries of the collection
...
\begin{figure}
	\includegraphics[width=\linewidth]{images/class_back-end.png}
...
\begin{figure}[h!]
	\centering
	\includegraphics[origin=c,max size={\textheight}{\textwidth}]{images/activity_ProcessData.png}
...
\begin{itemize}
\item{Move all (sub-)sections one numbering level up.}
\end{itemize}

\subsection{Changes to the front-end}
\begin{itemize}
\item{Update the login screen: add input field for server url.}
\end{itemize}
...
\subsection{Changes to the back-end}
\begin{itemize}
\item{Added new aggregator class AddressesAndLinks}
\item{Added class LoginFailureException}
\item{Rename class PacketRecord to PacketRecordDesFromKafka, changed the type of timestamp}
\item{Added class PacketRecordDesFromMongo}
\item{MongoClientMediator: added variants of methods addRecordToCollection() and addRecordsToCollection() to handle Records and bson Documents.}
\item{MongoClientMediator: added new method getTopicsForProcessing().}
\item{Hub: methods login(), authenticate() and logout() were added.}
\end{itemize}
