...
\begin{figure}[h!]
	\includegraphics[max size={\textwidth}{0.9\textheight}]{login2.pdf}
...
\begin{figure}[h!]
	...
	\caption{Selecting a specific option from a drow-down menu.}
\end{figure}
...
\autoref{fig:seq_server_init}  shows the initalization sequence order, the correspoding class diagram is \autoref{fig:class_back_end_init}, the program dependes on a couple of services namely (in order),the zookeper serivce, the kafka server service and the mongoDB service. Once all services are up and running the MongoConsumer is created and can start consuming messages and the Hub can start listening to client logins, requests, etc.
\begin{figure}[h!]
...
The admin role , created by default, can create and destroy users as well as assign specific roles to them. An analyst can see all collectoins on the database and an Operator can only see part of them.
